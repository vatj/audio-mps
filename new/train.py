...
def main(argv):
    ...
    with tf.compat.v1.variable_scope("model", reuse=tf.compat.v1.AUTO_REUSE):
        ...
        h_l2sqnorm = tf.reduce_sum(input_tensor=tf.square(model.freqs))
        r_l2sqnorm = tf.math.real(tf.reduce_sum(input_tensor=tf.math.conj(model.R) * model.R))
    ...
    with tf.compat.v1.variable_scope("summaries"):
        tf.compat.v1.summary.scalar("A", tf.cast(model.A, dtype=tf.float32))
        tf.compat.v1.summary.scalar("sigma", tf.cast(model.sigma, dtype=tf.float32))
        tf.compat.v1.summary.scalar("h_l2norm", tf.sqrt(h_l2sqnorm))
        tf.compat.v1.summary.scalar("r_l2norm", tf.sqrt(r_l2sqnorm))

        gr_rate = 2 * np.pi * hparams.sigma**2 * r_l2sqnorm / hparams.bond_dim
        tf.compat.v1.summary.scalar("gr_decay_time", 1 / gr_rate)

        tf.compat.v1.summary.scalar("model_loss", tf.reshape(model.loss, []))
        tf.compat.v1.summary.scalar("total_loss", tf.reshape(total_loss, []))

        tf.compat.v1.summary.audio("data", data, sample_rate=FLAGS.sample_rate, max_outputs=5)
        tf.compat.v1.summary.histogram("frequencies", model.freqs / (2 * np.pi))
        ...
        if FLAGS.visualize:
            ...
            tf.compat.v1.summary.image("data_waveform", data_waveform_op)
            ...
            if FLAGS.num_samples != 0:
                ...
                tf.compat.v1.summary.image("sample_waveform", sample_waveform_op)


    step = tf.compat.v1.get_variable("global_step", [], tf.int64, tf.zeros_initializer(), trainable=False)
    train_op = tf.compat.v1.train.AdamOptimizer(learning_rate=hparams.learning_rate).minimize(total_loss, global_step=step)
