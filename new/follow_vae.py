...
def build_loss_psi(data):
    ...
    loss = batch_zeros
    data = tf.transpose(a=data, perm=[1, 0])  # foldl goes along the first dimension
...
def _expectation_psi(psi):
    R = tf.compat.v1.get_variable(name="R", shape=[self.bond_d, self.bond_d], dtype=tf.float32, initializer=None)
    R_c = tf.cast(R, dtype=tf.complex64)
    exp = tf.einsum('ab,bc,ac->a', tf.math.conj(psi), R_c, psi)
    return 2 * tf.math.real(exp)
...
def model_fn(features, labels, mode, params, config):
  ...
  train_op = tf.compat.v1.train.AdamOptimizer(1e-3).minimize(loss, global_step=step)

  return tf.estimator.EstimatorSpec(
      mode=mode,
      loss=loss,
      train_op=train_op,
      eval_metric_ops={"loss": tf.compat.v1.metrics.mean(loss)})
...
def static_nsynth_dataset(directory):
  ...
  def _parser(example_proto):
      features = {"audio": tf.io.FixedLenFeature([2 ** 16], dtype=tf.float32)}
