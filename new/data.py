...
def get_audio(datadir, dataset, hps):
    ...
    if dataset == 'damped_sine':
        ...
        delays = tf.stack(input_length * [tf.random.gamma([hps.minibatch_size], alpha=2, beta=2/delay_time)], axis=-1)
        ...
        parse_function = lambda example_proto: \
            tf.io.parse_single_example(serialized=example_proto, features={"audio": tf.io.FixedLenFeature([FLAGS.sample_duration], dtype=tf.float32)})
