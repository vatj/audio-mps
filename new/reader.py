...
class NSynthDataset(object):
    ...
    def iterator(self, batch_size=32, length=2**16, repeat=False):
        ...
        dataset = dataset.apply(tf.data.experimental.shuffle_and_repeat(buffer_size=10000))
    ...
    def _parse_function(self, example_proto):
      features = {
            ...
            "instrument_source_str": tf.io.FixedLenFeature([1], dtype=tf.string),
            "instrument_family_str": tf.io.FixedLenFeature([1], dtype=tf.string),
            "audio": tf.io.FixedLenFeature([64000], dtype=tf.float32),
        }
      return tf.io.parse_single_example(serialized=example_proto, features=features)
