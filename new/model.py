...
class CMPS:
    ...
    def __init__(self, hparams, data_iterator=None, freqs_in=None, R_in=None):
        ...
        self.A = hparams.A
        self.A = tf.compat.v1.get_variable("A", dtype=tf.float32, initializer=hparams.A)
        ...
        if R_in is not None:
            Rx = tf.compat.v1.get_variable("Rx", dtype=tf.float32, initializer=R_in.real)
            Ry = tf.compat.v1.get_variable("Ry", dtype=tf.float32, initializer=R_in.imag)
        else:

            Rx = tf.math.rsqrt(self.r_reg) * tf.compat.v1.get_variable("Rx", shape=2*[self.bond_d], dtype=tf.float32,
                                                             initializer=tf.random_normal_initializer)
            Ry = tf.math.rsqrt(self.r_reg) * tf.compat.v1.get_variable("Ry", shape=2*[self.bond_d], dtype=tf.float32,
                                                             initializer=tf.random_normal_initializer)

        self.R = tf.complex(Rx, Ry)
        self.R -= tf.linalg.diag_part(self.R) # Remove diagonal part

        if freqs_in is not None:

            self.freqs = tf.compat.v1.get_variable("freqs", dtype=tf.float32, initializer=freqs_in)
        else:

            self.freqs = tf.math.rsqrt(self.h_reg) * tf.compat.v1.get_variable("freqs", shape=[self.bond_d], dtype=tf.float32,
                                                                initializer=tf.random_normal_initializer)
...
class RhoCMPS(CMPS):
    ...
    def rho_evolve_with_sampling(self, num_samples, length, temp=1):
        ...
        noise = tf.random.normal([length, num_samples], stddev=self.sigma * np.sqrt(temp * self.delta_t))
    ...
    def purity(self, num_samples, length, temp=1):
        ...
        return tf.math.real(tf.transpose(a=tf.linalg.trace(tf.einsum('abcd,abde->abce', rho, rho)), perm=[1, 0]))
    ...
    def sample(self, num_samples, length, temp=1):
        ...
        return self.A * tf.transpose(a=samples, perm=[1, 0])
    ...
    def _rho_init(self, W_in):
        if W_in is not None:
            Wx_in = W_in.real
            Wy_in = W_in.imag
            Wx = tf.compat.v1.get_variable("Wx", dtype=tf.float32, initializer=Wx_in)
            Wy = tf.compat.v1.get_variable("Wy", dtype=tf.float32, initializer=Wy_in)
        else:
            Wx = tf.compat.v1.get_variable("Wx", shape=[self.rank_rho_0, self.bond_d], dtype=tf.float32, initializer=None)
            Wy = tf.compat.v1.get_variable("Wy", shape=[self.rank_rho_0, self.bond_d], dtype=tf.float32, initializer=None)
        W = tf.complex(Wx, Wy)
        rho_0 = tf.matmul(W, W, adjoint_a=True)
        rho_0 = rho_0 / tf.linalg.trace(rho_0)
        return rho_0
    ...
    def _build_loss_rho(self):
        ...
        incs = self.data_iterator[:, 1:] - self.data_iterator[:, :-1]
        incs = tf.transpose(a=incs, perm=[1, 0])  # foldl goes along the 1st dimension
    ...
    def _expectation(self, rho, t):
        with tf.compat.v1.variable_scope("expectation"):
            t = tf.cast(t, dtype=tf.complex64)
            phases = tf.exp(1j * self.freqsc * t)
            Rt = tf.einsum('a,ab,b->ab', phases, self.R, tf.math.conj(phases))
            x = tf.add(Rt, tf.linalg.adjoint(Rt))
            exp = tf.linalg.trace(tf.einsum('ab,cbd->cad', x, rho))
            return tf.math.real(exp)

    def _normalize_rho(self, x, epsilon=1e-12):
        with tf.compat.v1.variable_scope("normalize"):
            tr = tf.reshape(tf.linalg.trace(x), [tf.shape(input=x)[0], 1, 1])
            x_inv_tr = tf.math.reciprocal(tf.maximum(tf.math.real(tr), epsilon))
...
class PsiCMPS(CMPS):
    ...
    def __init__(self, hparams, psi_in=None, *args, **kwargs):
        ...
        if psi_in is not None:
            psi_x = tf.compat.v1.get_variable("psi_x", dtype=tf.float32, initializer=psi_x_in)
            psi_y = tf.compat.v1.get_variable("psi_y", dtype=tf.float32, initializer=psi_y_in)
        else:
            psi_x = tf.compat.v1.get_variable("psi_x", shape=[self.bond_d], dtype=tf.float32, initializer=None)
            psi_y = tf.compat.v1.get_variable("psi_y", shape=[self.bond_d], dtype=tf.float32, initializer=None)
    ...
    def psi_evolve_with_data(self):
        ...
        return tf.transpose(a=psi, perm=[1, 0, 2])
    ...
    def _inc_loss_psi(self, psi, signal, t):
        return - tf.math.log(1. + self._expectation(psi, t) * signal / self.A)

    def _norm_square_psi(self, psi):
        exp = tf.einsum('ab,ab->a', tf.math.conj(psi), psi)
        return tf.math.real(exp)
    ...
    def _expectation(self, psi, t):
        with tf.compat.v1.variable_scope("expectation"):
            ...
            phases = tf.exp(1j * self.freqsc * t)
            Upsi = psi * tf.math.conj(phases)
            exp = tf.einsum('ab,bc,ac->a', tf.math.conj(Upsi), self.R, Upsi)
            return 2 * tf.math.real(exp)  # Conveniently returns a float
    ...
    def _normalize_psi(self, x, axis=None, epsilon=1e-12):
        ...
        with tf.compat.v1.variable_scope("normalize"):
            square_sum = tf.reduce_sum(input_tensor=tf.square(tf.abs(x)), axis=axis, keepdims=True)
            x_inv_norm = tf.math.rsqrt(tf.maximum(square_sum, epsilon))
