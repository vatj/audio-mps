...
class TestCMPS(tf.test.TestCase):
    ...
    def testRHasNoDiagonalElements(self):
        ...
        with self.cached_session() as sess:
            ...
            self.assertAllClose(tf.linalg.diag_part(model.R.eval()), hparams.bond_dim * [0.])
...
class TestRhoCMPS(tf.test.TestCase):
    ...
    def testRho0IsADensityMatrix(self):
        ...
        with self.cached_session() as sess:
            sess.run(tf.compat.v1.global_variables_initializer())
            self.assertAllClose(model.rho_0, model.rho_0 / tf.linalg.trace(model.rho_0))
            self.assertAllClose(model.rho_0, tf.transpose(a=model.rho_0, conjugate=True))
    ...
    def testRhoEvolvedWithDataRemainsNormalized(self):
        ...
        with self.cached_session() as sess:
            sess.run(tf.compat.v1.global_variables_initializer())
            self.assertAllClose(tf.linalg.trace(rho_out), tf.ones_like(rho_out[:,:,0,0]), rtol=1e-5)
    ...
    def testRhoEvolvedSamplingRemainsNormalized(self):
        ...
        with self.cached_session() as sess:
            sess.run(tf.compat.v1.global_variables_initializer())
            self.assertAllClose(tf.linalg.trace(rho_out), tf.ones([samples, FLAGS.sample_duration]), rtol=1e-4)
...
class TestPsiCMPS(tf.test.TestCase):
    ...
    def testPsiEvolvedWithDataRemainsNormalized(self):
        ...
        with self.cached_session() as sess:
            sess.run(tf.compat.v1.global_variables_initializer())
            self.assertAllClose(tf.norm(tensor=psi_out, axis=-1), tf.ones_like(psi_out[:,:,0]), rtol=1e-5)
